...
try:
    ...
    print("Warning: ctcdecode not available. Will use MaxDecode mode for decoding.")
import numpy as np
...
class Decode(object):
    def __init__(self, gloss_dict, num_classes, search_mode, blank_id=0):
        ...
        # Only initialize ctcdecode if beam search is requested and available
        if search_mode == "beam" and CTC_DECODE_AVAILABLE:
            vocab = [chr(x) for x in range(20000, 20000 + num_classes)]
            self.ctc_decoder = ctcdecode.CTCBeamDecoder(
                vocab, beam_width=10, blank_id=blank_id, num_processes=10
            )
        elif search_mode == "beam" and not CTC_DECODE_AVAILABLE:
            print("Warning: ctcdecode not available. Switching to 'max' decode mode.")
    ...
    def decode(self, nn_output, vid_lgt, batch_first=True, probs=False):
        ...
        if self.search_mode == "max":
            ...
            if not CTC_DECODE_AVAILABLE or self.ctc_decoder is None:
                print(
                    "Warning: Falling back to MaxDecode as ctcdecode is not available."
                )
                return self.MaxDecode(nn_output, vid_lgt)
